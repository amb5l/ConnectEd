...
class EditDuplicateInteraction(EditPasteInteraction):
    ...
    def __init__(
        self  : Self,
        view  : DiagramView,
        items : list[QGraphicsItem],  # items to duplicate
        pos   : QPointF               # duplication origin
    ) -> None:
        clone_items = []
        for item in items:
            if isinstance(item, ItemCloneMixin):
                clone_items.append(item.clone())
        if clone_items:
            DiagramItemsInteraction.__init__(self, view, clone_items)
...
class EditAdjustPolySegInteraction(PreviewStateMixin, DiagramInteraction):
    ...
    def update(self : Self, pos : QPointF) -> None:
        ...
        self._guide2.setLine(QLineF(center, corner or center))
        self._guide3.setLine(QLineF(corner or center, pos))
        # done
