...
class FillAppearanceLayout(QVBoxLayout):
    ...
    def __init__(
        ...
        self.color_combo = ColorComboBox(overrides.color, theme.color)
        ...
        self.style_combo = FillStyleComboBox(overrides.style, theme.style)
    ...
    def _onColorChanged(self : Self, _index : int = 0) -> None:
        ...
        if style is None: style = self._theme.style
        if style != Qt.BrushStyle.NoBrush: return
        auto_style = None if self._theme.style != Qt.BrushStyle.NoBrush else \
